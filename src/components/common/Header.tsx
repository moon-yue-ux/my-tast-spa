...
export default function Header() {
  ...
  const chainLabel = useMemo(() => currentNetwork?.name || (walletState.chainId ? `Chain ${walletState.chainId}` : '-'), [currentNetwork?.name, walletState.chainId]);
    ...
    <header className="sticky top-0 z-30 w-full backdrop-blur bg-white/70 border-b border-slate-200">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <Link to="/market" className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-800 tracking-tight">Web3 School</span>
        </Link>
          ...
          {isConnected ? (
            <>
              ...
              >
                ...
                <span className="px-2 py-0.5 text-xs rounded bg-white/20">{shortAddr(walletState.address)}</span>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-72 rounded-xl border border-slate-200 bg-white shadow p-4">
                  ...
                  <div className="font-mono text-xs mb-3 break-all">{walletState.address}</div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <InfoBox label="ChainId" value={walletState.chainId ?? '-'} />
                    <InfoBox label="ETH 余额" value={walletState.balance ?? '-'} />
                  </div>
              ...
              {connectMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-xl border border-slate-200 bg-white shadow p-2">
                  {connectors && connectors.length > 0 ? (
                    ...
                    <div className="px-3 py-2 text-sm text-slate-500">未发现可用钱包</div>
                  )}
                </div>
              )}
            </>
